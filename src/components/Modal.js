...
const Modal = ({ visible, handleOk, handleCancel }) => {
  ...
  const onSearchHandle = () => {
    advancedSearchNews(text, source, fromDate, toDate, language, sort);
    handleOk();
  };
  ...
  return (
    <AModal visible={visible} onCancel={() => handleCancel()} footer={null}>
      ...
      />
      <Menu key="advanceSearch">
        <Menu.ItemGroup title={'Advance Search'}>
          <Menu.Item key="Option">
            <Cascader
              options={renderSourcesOptions()}
              onChange={(value, selectedOptions) => setSource(value)}
          ...
          <Menu.Item>
            <Cascader
              options={sortBy}
              onChange={(value, selectedOptions) => setSort(value[0])}
              placeholder="Sort By"
              showSearch={{ filter }}
            />
          </Menu.Item>
